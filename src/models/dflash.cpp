...
llama_model_dflash::graph<false>::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        ggml_tensor * cur = use_iswa
            ? build_attn(inp_attn_iswa, wo,      NULL, gated ? NULL : layer.wo_s, Qcur, Kcur, Vcur, nullptr, gated ? nullptr : layer.attn_sinks, nullptr, kq_scale, il)
            : build_attn(inp_attn,      wo,      NULL, gated ? NULL : layer.wo_s, Qcur, Kcur, Vcur, nullptr, gated ? nullptr : layer.attn_sinks, nullptr, kq_scale, il);
